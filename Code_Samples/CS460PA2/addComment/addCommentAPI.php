...
// Function to check login
	public function addComment ($photoID, $name, $UserID){
		$this->mysqlconnect();
			...
			$row = $result->fetch_assoc() )
	{
		...
		if ($name==$captionez){
			...
			return ("Comment name  has already been used.");
			exit;
		}
		...
		$query = "INSERT INTO comments VALUES ('$UserID','$photoID', '$name',  '$date')";
...
$answer= "Person does not exits.";
	...
	return $answer;
	     ...
	     public function AlbumList($UserID){
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, comments P WHERE P.userid ='$UserID'AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
  ...
  return $this->ALBUMS;
		     ...
		     public function commentZList($UserID){
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, comments P WHERE P.userid ='$UserID'AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
  ...
  return $this->TAGS;
		     ...
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, comments P, user_table L WHERE P.userid ='$UserID' AND S.userid = L.userid AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
...
public function deleteComment($photoID, $name){
$this->mysqlconnect();

$query = "DELETE FROM comments WHERE photoid ='$photoID' AND `text`= '$name'";
$this->mysqli->query($query) or die("INSERT query failed: ".$this->mysql->error);
return 1;
