...
// Function to check login
	public function addTag ($photoID, $name, $UserID){
		$this->mysqlconnect();


		//Check for duplications in the table
	$query = "SELECT * FROM tags_table WHERE photoid = '$photoID'";
	...
	{
		$captionez = stripslashes($row["tag"]);
		...
		if ($name==$captionez){
			...
			return ("Tag name  has already been used.");
			exit;
		}
		...
		$query = "INSERT INTO tags_table VALUES ('$photoID', '$name',  '$UserID')";
...
$answer= "Person does not exits.";
	...
	return $answer;
	     ...
	     public function AlbumList($UserID){
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, tags_table P WHERE P.userid ='$UserID'AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
  ...
  return $this->ALBUMS;
		     ...
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, tags_table P, user_table L WHERE P.userid ='$UserID' AND S.userid = L.userid AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
  ...
  return $this->PEOPLE;
		     ...
		     public function tagZList($UserID){
		     $this->mysqlconnect();
			$query = "SELECT * FROM albums S, photos N, tags_table P WHERE P.userid ='$UserID'AND S.albumid = N.albumid AND P.photoid = N.photoid ";
			$id=0;
...
public function deleteTag($photoID, $name){
$this->mysqlconnect();

$query = "DELETE FROM tags_table WHERE photoid ='$photoID' AND tag = '$name'";
$this->mysqli->query($query) or die("INSERT query failed: ".$this->mysql->error);
return 1;
