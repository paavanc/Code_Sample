<?php
//echo "jugllu";
include 'addLikeStub.php';
include 'addLikeAPI.php';

//include '../Login/login.php';
class addLikeActivity
{
...
<?php
     ...
     $this->likeList=$model->likeZList($_SESSION['userid']);
      ...
      $this->arrlength=count($this->albumList);
        ...
        echo"<table id= 'myTable' style='width:800px;'>";
           ...
           echo "<td> <form  action='' method='POST'>"; 
            ...
            echo "<input type='submit' id=" .$this->counter. " name='Deletez' value='Unlike' ></form></td>";
            echo "<td> <form  action='' method='POST'>";
...
?>
	...
	private function verifyForm(){
		...
		if((!isset($this->albumName) || $this->albumName == '') || (!isset($this->pictureName) || $this->pictureName == '') || (!isset($this->userName) || $this->userName == '') ){
			//something's missing
			$this->PrintError(0);
	...
	private function processData(){
		if($this->context == "Data was submitted"){
          ...
          $pictureID=$model->getPhotoID($albumID, $this->pictureName);
					...
					$result = $model->addLike($pictureID, '1', $_SESSION['userid'] );
     ...
     $this->likeList=$model->likeZList($_SESSION['userid']);
      ...
      $result=$model->deleteLike($this->idphoto[$this->del], $this->likeList[$this->del]);
      if($result==1){



print '<script type="text/javascript">'; 
print 'alert("The like '.$this->likeList[$this->del].' from the picture '. $this->captionList[$this->del].' from Album '. $this->AlbumList[$this->del]. ' is succesfuly deleted")'; 
print '</script>'; 
                ...
                print '<script type="text/javascript">'; 
print 'alert("The like '.$this->likeList[$this->del].' from the picture '. $this->captionList[$this->del].' from Album '.$this->AlbumList[$this->del].' is NOT succesfuly  DELETED")'; 
print '</script>'; 
      }
